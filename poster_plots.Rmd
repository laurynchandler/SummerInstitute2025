...
# fit regression model for each subject, extract single slope for RTs across all cues in learning
```{r}
learning_coefs <- list()
...
summary_df <- df_inf %>% 
  group_by(subID, congCue, trueCongruence, accuracyFactor) %>%
...
```{r}
# simple linear regression (same fixed effect for all subjects)
...
rt_lm_zScore <- df_inf %>% mutate(logRT = scale(log(RT))) %>%
...
# Save plot
ggsave(paste0(outdir, 'infRts_byAccuracy.png'), width = 8, height = 3, dpi = 600, units = "in")
```

## how accuracy, logTrial.trend, and congCue interact to shape zconfidence ratings

## updated approach as of 08-07-2025
```{r}

rt_confidence_zScore <- df_inf %>% mutate(confidence = scale(confidence)) %>%
  lmer(confidence ~ congCue * simpleTrend + (congCue*simpleTrend | subID),.,
      control = lmerControl(optimizer='bobyqa'))

summary(rt_confidence_zScore)

rt_conf_plot_df <- emmip(rt_confidence_zScore, congCue ~ simpleTrend , plotit=F, CIs = T, at=list(simpleTrend = unique(df_inf$simpleTrend),
                                                                  congCue = unique(df_inf$congCue)))
```
