...
# cuecorr lollipop plot
```{r}
# Prepare the data
all_participants <- sort(unique(df_est$subID))
conditions <- c("condition_80cue", "condition_65cue")  # Define your specific conditions

# Create plotting data with position adjustments
plot_data <- df_est %>%
  select(subID, condition, cueCorr) %>%
  distinct() %>%
  arrange(subID) %>%
  mutate(
    participant_index = match(subID, all_participants),
    condition_offset = ifelse(condition == "condition_80cue", -0.15, 0.15),
    x_position = participant_index + condition_offset
  )

# Create the plot
p <- ggplot(plot_data, aes(x = x_position, y = cueCorr, color = condition)) +
  # Add vertical lines from 0 to each point
  geom_segment(aes(xend = x_position, yend = 0), 
               linewidth = 1.5, alpha = 0.8) +
  # Add points
  geom_point(size = 3) +
  # Customize colors (using default ggplot2 colors)
  # scale_color_discrete() +
  # Set x-axis
  scale_x_continuous(
    breaks = seq(1, length(all_participants), by = 2),  # Show every 2nd participant
    labels = all_participants[seq(1, length(all_participants), by = 2)],
    limits = c(0.7, length(all_participants) + 0.3),  # Tighter limits
    expand = c(0, 0) 
  ) +
  # Labels and title
  labs(
    x = "SubID",
    y = "Cue Correlation",
    title = "Cue Correlation by SubID and Condition",
    color = "Condition"
  ) +
  # Theme adjustments
  theme_minimal() +
  theme(
    ...
    legend.position = "right",  # Ensure legend appears on the right side
    figure.size = c(14, 8)
  )
p

# Save the plot
ggsave("~/Desktop/SummerInstitute2025/plots/cueCorr_by_condition.png", plot = p, width = 14, height = 8, dpi = 600, units = "in")
```


# slope of regression lines during learning: all subjects

## predicting z-scored image-locked RT using `trueCue * logTrial`, controlling for response finger using `imageIdx` as a regressor
```{r}
# fit model
m <- lm(scaledImgLockedRT ~ logTrial * trueCue + imageIdx, data = df_learn)
# plot trends
emmip(m, trueCue ~ logTrial, CIs=T, at=list(trueCue = unique(df_learn$trueCue),
                                                    logTrial = range(df_learn$logTrial))) + 
  geom_hline(yintercept=mean(df_learn$scaledImgLockedRT,na.rm=T)) + labs(y='scaledimgLockedRT', title = 'fitted trends: scaledimgLockedRT ~ logTrial * trueCue')
# view summary
summary(m) 
```
...
# empirical plots: scaledImgLockedRT by logTrial
```{r, include=F}
df_learn %>%
  mutate(trueCue = factor(trueCue))%>%
  ggplot(aes(x=logTrial, y=scaledImgLockedRT, color=trueCue, fill=trueCue)) +
...
df_trueCue_interaction_coefs <- df_trueCue_coefs %>% filter(term == 'logTrial:trueCue') 
...
df_cueDiffs %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  ...
  ggplot(., aes(x=logTrial.trend, y=meanDiff, color=trueCue, fill=trueCue)) + facet_wrap(~trueCue) + geom_hline(yintercept=0) +
...
# fit a linear model
df_cueDiffs %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  lm(meanDiff ~ logTrial.trend*trueCue, .) %>% summary()
```
...
df_est %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  mutate(trueCue = factor(trueCue, levels=c(0.5, 0.65, 0.8))) %>%
  ggplot(., aes(x=logTrial.trend, y=subjectiveCue, color=trueCue, fill=trueCue)) + facet_wrap(~trueCue) + 
...
# fit linear model
df_est %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  lm(subjectiveCue ~ logTrial.trend*trueCue, .) %>% summary()

# compute correlation
df_est %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  select(logTrial.trend, trueCue, subjectiveCue) %>%
  group_by(trueCue) %>%
  summarise(linear_corr = cor(logTrial.trend, subjectiveCue, method='pearson'),
            nonlinear_corr = cor(logTrial.trend, subjectiveCue, method='spearman'))
```
...
df_est %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  mutate(trueCue = factor(trueCue, levels=c(0.5, 0.65, 0.8))) %>%
  ggplot(., aes(x=logTrial.trend, y=zCueConf, color=trueCue, fill=trueCue)) + geom_hline(yintercept = 0) +
  facet_wrap(~trueCue) + 
...
# fit linear model
df_est %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  lm(zCueConf ~ logTrial.trend*trueCue, .) %>% summary()

# compute correlation
df_est %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  select(logTrial.trend, trueCue, zCueConf) %>%
  group_by(trueCue) %>%
  summarise(linear_corr = cor(logTrial.trend, zCueConf, method='pearson', use='na.or.complete'),
            nonlinear_corr = cor(logTrial.trend, zCueConf, method='spearman', use='na.or.complete'))
```
...
df_inf %>% 
  ...
  mutate(trueCue = factor(trueCue, levels=c(0.5, 0.65, 0.8))) %>%
  ggplot(., aes(x=logTrial.trend, y=meanRT, color=factor(congCue), fill=factor(congCue))) + geom_hline(yintercept = 0) +
  facet_grid(~congCue) + 
  theme_bw() + geom_point() + stat_smooth(method = 'lm') + labs(y = 'RTs during inference (decision making)', x = 'learning RT slope', title = 'effects on RTs independent of accuracy')

# fit & plot linear model
# df_inf %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
#   lm(zlogRT ~ logTrial.trend*congCue, .) %>% #summary()
#   emmip(., ~ logTrial.trend | congCue, var='congCue', CIs=T,
#         at=list(logTrial.trend=range(df_trueCue_trends$logTrial.trend),
#                                                                     congCue = c(0.2, 0.5, 0.8)))
# # emtrends for linear model
# df_inf %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
#   lm(zlogRT ~ logTrial.trend*congCue, .) %>%
#   emtrends(eff ~ logTrial.trend, var = 'congCue', 
#            at=list(logTrial.trend=range(df_trueCue_trends$logTrial.trend),
#                    congCue=c(0.8)))
```
...
df_inf %>% 
  ...
  ggplot(., aes(x=logTrial.trend, y=meanRT, color=factor(congCue), fill=factor(congCue))) + geom_hline(yintercept = 0) +
  facet_grid(~congCue) + 
  theme_bw() + geom_point(aes(shape=accuracyFactor)) + stat_smooth(aes(linetype=accuracyFactor), method = 'lm') + labs(y = 'RTs during inference (decision making)', x = 'learning RT slope', title = 'inference RTs by accuracy')

#Save plot
ggsave("~/Desktop/SummerInstitute2025/plots/infRts_byAccuracy.png", width = 14, height = 8, dpi = 600, units = "in")
```
...
## how accuracy, logTrial.trend, and congCue interact to shape zconfidence ratings

```{r}
df_inf %>% 
  mutate(accuracyFactor = factor(accuracy, levels=c(1,0), labels=c('correct', 'incorrect'))) %>%
  group_by(subID, congCue, trueCue, trueCongruence, accuracyFactor) %>%
  summarise(meanRate = mean(zconf, na.rm=T)) %>%
  left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  mutate(trueCue = factor(trueCue, levels=c(0.5, 0.65, 0.8))) %>%
  ggplot(., aes(x=logTrial.trend, y=meanRate, color=factor(congCue), fill=factor(congCue))) + geom_hline(yintercept = 0) +
  facet_grid(~congCue) + 
  theme_bw() + geom_point(aes(shape=accuracyFactor)) + stat_smooth(aes(linetype=accuracyFactor), method = 'lm') + labs(y = 'confidence ratings during inference (decision making)', x = 'learning RT slope', title = 'zconfidence ratings by accuracy')
```

incongruent cues they get more confident with correct responses
reverse of trends
slower during learning more confident during incorrect responses
fast during lower slightly more confident in incorrect decisions 

## how accuracy, logTrial.trend, and congCue interact to shape zconfidence reaction times
```{r}
df_inf %>% 
  mutate(accuracyFactor = factor(accuracy, levels=c(1,0), labels=c('correct', 'incorrect'))) %>%
  group_by(subID, congCue, trueCue, trueCongruence, accuracyFactor) %>%
  summarise(meanZconfRT = mean(zconfRT, na.rm=T)) %>%
  left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  mutate(trueCue = factor(trueCue, levels=c(0.5, 0.65, 0.8))) %>%
  ggplot(., aes(x=logTrial.trend, y=meanZconfRT, color=factor(congCue), fill=factor(congCue))) + geom_hline(yintercept = 0) +
  facet_grid(~congCue) + 
  theme_bw() + geom_point(aes(shape=accuracyFactor)) + stat_smooth(aes(linetype=accuracyFactor), method = 'lm') + labs(y = 'confidence ratings RTS during inference (decision making)', x = 'learning RT slope', title = 'confidence rating RTs by accuracy')
```
...
# Learning RTs and Cue Prediction Accuracy
```{r}
df_learn %>%
  left_join(df_trueCue_trends, by = c('subID', 'trueCue')) %>%
  mutate(congCueFactor = factor(congCue, levels=c(0,1), labels=c('incongruent', 'congruent'))) %>%
  ggplot(., aes(x=logTrial.trend, y=accuracy, color=factor(congCue), fill=factor(congCue))) +
  geom_hline(yintercept = 0.5) +
  facet_grid(~congCue) + 
  theme_bw() + 
  stat_smooth(method = 'lm', se = TRUE, alpha = 0.2) + 
  labs(
    y = 'Cue Prediction Accuracy', 
    x = 'Learning RT Slope', 
    title = 'Learning RTs and Cue Prediction Accuracy'
  ) +
