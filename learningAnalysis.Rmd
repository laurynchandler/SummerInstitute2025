...
df_est %>% select(cueCorr, subID) %>% unique() %>%
  ...
  stat_smooth(method='lm') + labs(x='scaledLogTrial*trueCue interaction coefficient')
...
df_cueDiffs %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  ...
  ggplot(., aes(x=scaledLogTrial.trend, y=meanDiff, color=trueCue, fill=trueCue)) + facet_wrap(~trueCue) + 
  theme_bw() + geom_point() + stat_smooth(method = 'lm') + labs(y = 'average cueDiff', x = 'effect of trial on scaledImgLockedRT')

# fit a linear model
df_cueDiffs %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  lm(meanDiff ~ scaledLogTrial.trend*trueCue, .) %>% summary()
```
## correlation between simple slope for each trueCue and the reported value for that cue
```{r}
# plot
df_est %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  ...
  theme_bw() + geom_point() + stat_smooth(method = 'lm') + labs(y = 'subjectiveCue', x = 'slope of scaledLogTrial')
...
## correlation between simple slope for each trueCue and the average confidence for that cue
```{r}
# plot
df_est %>% left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  mutate(trueCue = factor(trueCue, levels=c(0.5, 0.65, 0.8))) %>%
  ...
  facet_wrap(~trueCue) + 
  theme_bw() + geom_point() + stat_smooth(method = 'lm') + labs(y = 'zCueConfidence', x = 'slope of scaledLogTrial')
