...
## how accuracy, scaledLogTrial.trend, and congCue interact to shape zconfidence ratings
...
df_inf %>% 
  ...
  left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  mutate(trueCue = factor(trueCue, levels=c(0.5, 0.65, 0.8))) %>%
  ggplot(., aes(x=scaledLogTrial.trend, y=meanRate, color=factor(congCue), fill=factor(congCue))) + geom_hline(yintercept = 0) +
  facet_grid(~congCue) + 
...
## how accuracy, scaledLogTrial.trend, and congCue interact to shape zconfidence reaction times
```{r}
df_inf %>% 
  mutate(accuracyFactor = factor(accuracy, levels=c(1,0), labels=c('correct', 'incorrect'))) %>%
  group_by(subID, congCue, trueCue, trueCongruence, accuracyFactor) %>%
  ...
  left_join(., df_trueCue_trends, by=c('subID', 'trueCue')) %>%
  mutate(trueCue = factor(trueCue, levels=c(0.5, 0.65, 0.8))) %>%
  ggplot(., aes(x=scaledLogTrial.trend, y=meanZconfRT, color=factor(congCue), fill=factor(congCue))) + geom_hline(yintercept = 0) +
  facet_grid(~congCue) + 
