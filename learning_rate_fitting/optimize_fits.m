...
gridstep = 0.05;
search_space  = [-gridstep 1+gridstep];     % initial values for the model parameters

%% iterate through alpha types & subjects
fit_functions = {'generate_regressors_simpleAlpha', 'generate_regressors_scaledAlpha'};

for i = 2:length(fit_functions)
    fitfunc = fit_functions{i};
    for sub = unique(behav_data.subID)'
        ...
        thisfit = @(x) fit_function(fitfunc, sub_data, x);
        ...
        for k = min(search_space):gridstep:max(search_space)
            ...
            try
                if i==1
                    ...
                    params = [alpha_0 alpha_slope];
                    [temp_best_params, temp_loglik] = fmincon(thisfit, params, [], [], [], [], -Inf, Inf, [], options);
                end
            catch xcept
                disp('fmincon error');
                xcept
            ...
            if (temp_loglik < loglikelihood-(1e-32))
                temp_best_params(1) = 1./(1 + exp(-temp_best_params));   % back-transform alpha from logistic
    ...
    %% tidy & write out fits
    fits = rmmissing(fits);
    fits = fits(fits.subID > 0, :);
    writetable(fits, [extractAfter(fitfunc, 'generate_regressors_') '_fits.csv']);
end % for each form of alpha